    ...
    exportXLSX() {
        ...
        headers.forEach((header, index) => {
            ...
            worksheet[cellAddress] = {
                v: header,
                s: { alignment: { wrapText: true, vertical: "center", horizontal: "center" } }
            };
        });
        ...
        this.table.forEach((row, rowIndex) => {
            const values = [
                this.formatDate(row.inputDate),
                row.contractor.name,
                row.destination,
                row.initiator.name,
                row.sum,
                row.sumClosing,
                this.dd.control[this.control(row)].text,
                row.about.text,
                row.mark.text,
                row.status.text,
                this.formatDate(row.copyDate) === '01.01.1970' ? '' : this.formatDate(row.copyDate),
                this.formatDate(row.origDate) === '01.01.1970' ? '' : this.formatDate(row.origDate),
                row.title,
            ];
